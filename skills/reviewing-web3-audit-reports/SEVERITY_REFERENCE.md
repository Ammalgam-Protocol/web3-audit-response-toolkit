...
| **High** | Significant financial loss (>1% principal, >$10), major protocol disruption | Exploitable without extensive external conditions | Theft of unclaimed yield, temporary fund freezing, loss >1% AND >$10 (Sherlock), high impact + high/medium likelihood (Cantina) |
| **Medium** | Constrained loss, broken core functionality, operational disruption | Requires specific external conditions or states | Loss >0.01% AND >$10 (Sherlock), DOS >7 days, griefing, block stuffing, high impact + low likelihood OR medium impact + medium likelihood (Cantina) |
...
1. **Impact over Likelihood** (Sherlock): Severity driven by impact regardless of probability
2. **Impact x Likelihood Matrix** (Cantina): 3x3 matrix combining impact and likelihood
3. **Protocol Continuity Test** (Cantina): Protocol can function without fixes → likely Low
4. **Cumulative Damage** (Sherlock): Repeatable attacks causing cumulative losses can elevate severity
5. **Privilege Requirements** (Immunefi): Exploits requiring elevated privileges may be downgraded
6. **Professional Judgment** (Zellic): Case-by-case considering threat models and project context
